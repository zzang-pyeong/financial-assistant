...
**문맥 스니펫 추출** (`lib/sec_filings.py::_stream_find_context`): 검색 API가 스니펫을
주지 않아 공시 문서를 직접 받아 회사명 주변 ±180자를 잘라낸다. 문서를 통째로 받지 않고
`requests`의 `stream=True`로 청크(64KB)씩 받다가 **회사명을 찾는 즉시 연결을 끊는다** —
10-K 본문이 약 1.9MB인데(NVDA 2026 10-K 실측; 11MB로 표시되는 건 XBRL 포함 complete
submission 파일이다) 실제로 쓰는 건 수백 자뿐이기 때문이다. 가짜 10-K로 검증한 결과
문서의 2.7%만 읽고 중단했다. 청크 경계에서 태그가 반토막 나는 것(`_safe_split_point`),
멀티바이트 문자가 깨지는 것(`codecs` 증분 디코더)은 각각 따로 처리한다. 회사명을 못 찾은
채 6MB를 넘겨 읽으면 포기한다.
...
— 비용 없음). 그래프에 실제로 그려지는 상위 10개 + 허브만 조회하고(표에는 안 쓰므로 전체를
받을 이유가 없다), `ThreadPoolExecutor`로 병렬 처리한 뒤 프로세스 수명 동안 캐시한다.
캐시가 `st.cache_data`가 아닌 모듈 dict인 이유는 워커 스레드 호출 때문으로, 번역·스니펫과
같다.

**로고를 원형으로 잘라 원을 채운다** (`lib/logos.py`): plotly의 layout image는 사각형으로만
배치되고 원형 클리핑을 지원하지 않아서, 로고를 원 안에 내접시키면 원 가장자리가 늘 비어
보인다. 원을 채우려면 **이미지 자체가 원형**이어야 하므로, 받아온 로고를 가운데 정사각형으로
자르고 원형 알파 마스크를 씌운 PNG로 다시 만들어 data URI로 넘긴다(마스크는 4배 크기로
그린 뒤 축소해 둘레를 부드럽게 한다). 검증에서 알파>0 픽셀의 면적비가 0.785(=π/4)로 나와
정확히 원임을 확인했다. 투명 배경 로고는 흰 바탕을 먼저 깔아 원 안이 비어 보이지 않게 한다.

plotly shape path로 "사각형 빼기 원" 마스크를 덮는 방법도 검토했으나 버렸다 — shape path가
SVG 원호(A) 명령을 지원하지 않아 베지어 근사가 필요하고, 무엇보다 shape과 image 중 어느
쪽이 위에 그려지는지에 의존하는데 그건 plotly 내부 레이어 순서라 보장할 수 없다.

**가로로 긴 워드마크형 로고(비율 1.6 초과)는 자르지 않는다** — 정사각 아이콘형은 잘라도
멀쩡하지만, 회사 이름을 글자로 쓴 로고를 원으로 자르면 글자 두어 개만 남아 오히려 못
알아본다. 이 경우는 비율을 지켜 원 안에 내접시킨다(원이 꽉 안 차는 대신 로고가 온전하다).

로고 크기는 원형일 때 원 지름의 92%다 — 100%로 하면 이미지가 유형별 색 테두리 링을 덮어
버린다(plotly가 image를 shape보다 위에 그리기 때문). PIL이 못 읽는 형식(SVG 등)이면 원본
URL을 그대로 브라우저에 넘기고, 네모난 이미지이므로 원에 내접(지름의 70%)시킨다.

**로고가 아예 없는 것도 정상 경로다** — Finnhub가 로고를 못 주는 회사가 소형주에 흔하다.
이때는 로고 도입 전과 똑같이 빈 원으로 그려진다(plotly는 이미지 로딩에 실패해도 조용히
아무것도 안 그리므로, URL이 죽어도 같은 결과로 수렴한다). 티커 라벨은 로고 유무와 무관하게
항상 원 바깥에 붙인다 — 로고만으로는 어느 회사인지 모르는 경우가 많고, 라벨 위치가 노드마다
달라지면 눈으로 훑을 때 줄이 안 맞기 때문이다.

로고를 넣으면서 **노드 원을 픽셀 단위 marker에서 데이터 좌표 shape으로 바꿨다.** plotly의
layout image는 데이터 좌표로 배치되는데 marker는 픽셀이라, 둘을 섞으면 창 크기가 바뀔 때
로고가 원 밖으로 삐져나온다. 원이 채워지면서 **엣지 선도 노드 중심이 아니라 양쪽 원
테두리에서 끊도록** 바꿨다(`_edge_endpoints`) — 중심까지 그으면 선이 원 내부를 가로질러
로고 위에 겹친다. 원이 비어 있던 시절엔 안 보이던 문제다.

data URI는 Streamlit이 rerun마다 그림 JSON에 담아 브라우저로 보내므로 크기를 96px로
제한했다(로고 1개 약 3~8KB, 노드 10개 기준 수십 KB 수준).

**그래프 배치와 표기** (`lib/charts.py`): 노드는 12시 방향에서 시계방향으로 배치하고,
6개 이상이면 반지름을 번갈아 바꿔(1.0 / 1.24) 라벨 겹침을 줄인다(10개 기준 노드 간
최소거리 0.618 → 0.729). 라벨은 허브 반대쪽(위쪽 노드는 위, 아래쪽 노드는 아래)에
붙여 허브로 모이는 선과 겹치지 않게 한다. 유형별 색은 선과 노드 테두리·옅은 채움에
...
확대로 먹혔다. 관계도는 원형 배치라 좌표에 의미가 없어 확대·이동해서 볼 것도 없다.
`staticPlot=True`는 hover까지 죽이므로 쓰지 않고 개별 옵션으로 끈다.

**hover 텍스트는 직접 줄바꿈한다** (`_wrap_hover`): plotly는 hover 텍스트를 자동
줄바꿈하지 않아서, 긴 줄이 하나 있으면 말풍선이 그림 영역보다 넓어져 화면 밖으로 잘린다
(영문 장문인 SEC 발췌문에서 실제로 발생). 단어 경계로 끊고, 한글·한자는 라틴 문자의 두 배
폭으로 계산하며(한글 설명과 영문 발췌문이 섞여 있다), 공백이 없는 초장문 토큰은 폭에 맞춰
강제로 자른다. hover에 넣는 발췌문은 150자·2건까지만 — 전체는 아래 표가 보여준다.

**근거 표** (`pages/8_관계도.py`): 그래프 아래에 상대기업별 요약표(그래프에 안 그려진
회사까지 전부, 그래프 표시 여부를 ○로 구분)와 근거 전체 표(날짜·소스·상태·발췌문·원문
링크)를 `st.dataframe` + `LinkColumn`으로 노출한다. 예전엔 이 정보가 그래프 hover 안에만
있어서 원문 링크를 클릭할 수 없었고, 터치 환경에서는 hover 자체가 안 떴다. 그래프와 표가
같은 그룹핑·정렬을 쓰도록 `group_relationship_edges()`를 `lib/charts.py`에 한 번만 두고
양쪽이 공유한다. 그래프에 상위 10개만 그린다는 사실과 잘린 개수도 화면에 명시한다.
...
번역은 **헤드라인 1건당 HTTP 1회**다. `translate_batch()`는 이름과 달리 여러 문장을 한
요청으로 묶지 않고 내부에서 for문으로 `translate()`를 반복할 뿐이라(deep_translator
1.11.4 `base.py` 확인) 요청 수 절감에 쓸 수 없다. 그래서 요청 수를 줄이는 대신
`prefetch_korean()`이 `ThreadPoolExecutor`(워커 6개)로 **동시에** 보낸다.

호출 위치가 중요하다 — `fetch_and_store_ticker()`가 데이터 수집을 마친 직후 스피너 안에서
`prefetch_korean()`을 부르고, 그 뒤 `to_korean()`은 캐시만 읽는다. 예전엔 이 번역이
Conflict Board **렌더 도중**에 순차로 일어나서, 수집 스피너가 끝난 뒤 아무 안내 없이
화면이 굳은 것처럼 보였다. Analyst News·Company Events 페이지도 같은 패턴을 쓴다.

캐시는 `st.cache_data`가 아니라 상한(5,000건) 있는 모듈 수준 dict이다. 워커 스레드에서
`st.cache_data` 함수를 부르면 ScriptRunContext 경고가 나고, 무엇보다 배치로 미리 채운
결과가 개별 호출의 캐시 키와 안 맞아 캐시 미스가 나기 때문이다.

**표시 개수와 선반영 개수는 반드시 같아야 한다** — 어긋나면 초과분이 렌더 중에 다시
순차 요청을 일으킨다. 그래서 개수를 `lib/config.py`의 `BOARD_NEWS_LIMIT` /
`ANALYST_NEWS_DISPLAY_LIMIT` / `CORPORATE_EVENT_DISPLAY_LIMIT` 한 곳에 모아뒀다.
...
- **peer 정보 수집이 순차 루프다** — `classify_peers()`가 Finnhub가 준 peer 티커
  (보통 10~30개)마다 `yf.Ticker(p).info`를 for문으로 하나씩 긁는다. yfinance의 `.info`는
  비공식 스크래핑이라 건당 0.3~1.5초로 느리고, 이게 전부 첫 검색의 대기 시간에 들어간다.
  번역과 SEC 조회는 병렬화했지만 여기는 아직 손대지 않았다(다음 순위 개선 후보).
- 관계도 커버리지 확장은 검토 후 보류 상태다 — `논의_관계도_커버리지확장.md` 참조.
  Exhibit 21(자회사 목록)과 첨부문서 설명 활용안은 실측으로 기각했고(NVDA 10-K에
  EX-21은 3건뿐이고 EX-10은 아예 없음), 탐색 방향을 뒤집는 안(공시를 한 번 받아
  `company_tickers.json`의 전체 등록사 이름과 로컬 매칭)은 오탐 검증을 못 해 보류했다.
...
- `논의_관계도_커버리지확장.md` — 커버리지 확장 3안 검토 결과와 보류 근거(미구현)
