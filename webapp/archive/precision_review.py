"""보류된 기능 — '정밀 검토' 단계별 흐름 (2026-07-27 app.py에서 분리, 비활성화).

Conflict Board(비교 화면)만으로 충분하다고 판단해 8단계 강제 흐름(포지션 선택 →
반대 관점 강제 노출 → 지지 관점 → Conflict Board → 손절/익절 → 메모 → 최종 확인 →
매매일지 기록)을 프로그램에서 뺐다. 이 파일은 어디서도 import되지 않고, `pages/`
디렉터리 밖에 있어 Streamlit이 사이드바에 자동 등록하지도 않는다 — 즉 지금은 코드가
그냥 저장만 되어 있고 실행되지 않는다.

재활성화하려면:
1. 이 모듈을 app.py에서 `from archive.precision_review import render_precision_review`로 import
2. app.py의 "compare" 화면에 "🔍 정밀 검토 시작" 버튼을 되살려 `goto("intent")` 호출
3. app.py의 step-dispatch 체인(if/elif) 안에 `else: render_precision_review()`를 추가
   (이 모듈의 render_precision_review()가 "intent"/2~8 단계를 전부 처리함)

주의: 이 파일은 app.py와 별개로 완전히 독립적으로 동작하도록 필요한 헬퍼 함수
(compute_flagged_context, collect_bull_bear_lines 등)를 app.py 것과 별도로 복제해
갖고 있다. app.py를 import하면 그 순간 Streamlit 화면이 그려지므로 절대 app.py를
import하지 않는다.

⚠️ 2026-07-27 추가 삭제: 매매일지 기능 자체를 폐기하기로 하면서 `lib/journal.py`와
`trade_journal.csv`도 함께 삭제했다. 아래 STEP 7/8은 그 모듈의 `append_entry`/
`load_journal`에 의존하므로, 이 흐름을 재활성화하려면 그 두 함수를 git 이력
(07b80b6 이전 커밋)에서 복원하거나 다른 저장 방식으로 다시 짜야 한다 — 지금 이 파일
안의 import는 더 이상 유효하지 않다.
"""
...
def render_precision_review():
    """intent/2~8 단계 전체를 처리 — app.py의 step-dispatch 체인에서
    "search"/"compare"가 아닐 때 호출하도록 되어 있었다."""
