...
if own["institutions_pct"] is not None:
    summary_rows.append({
        ...
        "해석": institution_pct_interpretation(own["institutions_pct"]),
    })
if own["insiders_pct"] is not None:
    summary_rows.append({
        "지표": "내부자 보유율", "값": f"{own['insiders_pct']*100:.1f}%",
        "해석": insider_pct_interpretation(own["insiders_pct"]),
    })
if own["institutions_pct"] is not None and own["insiders_pct"] is not None:
    ...
    summary_rows.append({
        "지표": "기관+내부자 합계", "값": f"{combined*100:.1f}%",
        "해석": (
...
if own["short_pct_float"] is not None:
    summary_rows.append({
        ...
        "해석": "높을수록 하락 베팅 비중이 크다는 뜻이지만, 반대로 숏스퀴즈(급반등) 가능성도 있음",
    })
if own["float_ratio"] is not None:
    ...
    summary_rows.append({"지표": float_label, "값": f"{own['float_ratio']*100:.1f}%", "해석": interp})
if fund_ap_preview:
    ...
    if total > 0:
        summary_rows.append({
            ...
            "해석": "패시브 비중이 높을수록 지수 편입/이탈에 따른 기계적 매매가 많고, 펀더멘털과 무관한 수급 영향이 큼",
        })
if direction_preview:
    summary_rows.append({
        "지표": "내부자 매매 방향성", "값": direction_preview["direction"],
        "해석": "단순 보유율(%)보다 최근 실제 매수/매도 방향이 더 중요한 신호일 때가 많음",
    })
